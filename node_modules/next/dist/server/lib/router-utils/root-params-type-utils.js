...
async function writeRootParamsTypes(manifest, filePath, config) {
    const rootParams = manifest.rootParams;
    const featureEnabled = !!config.experimental.rootParams || !!config.cacheComponents;
    if (!featureEnabled) {
        await _fs.default.promises.rm(filePath, {
            force: true
        });
        return;
    }
