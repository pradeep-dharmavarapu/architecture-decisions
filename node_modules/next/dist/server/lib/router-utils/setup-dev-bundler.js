...
const _rootparamstypeutils = require("./root-params-type-utils");
const _fileclassifier = require("../../../build/file-classifier");
...
async function verifyTypeScript(opts) {
    const verifyResult = await (0, _verifytypescriptsetup.verifyAndRunTypeScript)({
        dir: opts.dir,
        ...
        rootParams: !!opts.nextConfig.experimental.rootParams || !!opts.nextConfig.cacheComponents
    });
...
async function startWatcher(opts) {
    ...
    if (opts.nextConfig.experimental.lockDistDir) {
        ...
        lockfile = await _lockfile.Lockfile.acquireWithRetriesOrExit(_path.default.join(distDir, 'lock'), 'next dev', true, JSON.stringify(serverInfo), opts.dir, opts.nextConfig.distDir);
    }
    ...
    const hotReloader = opts.turbo ? await (async ()=>{
        ...
        return await createHotReloaderTurbopack(opts, serverFields, distDir, resetFetch, lockfile, opts.serverFastRefresh);
    })() : await (async ()=>{
    ...
    await _fs.default.promises.writeFile(prerenderManifestPath, JSON.stringify({
        version: 4,
        routes: {},
        dynamicRoutes: {},
        notFoundRoutes: [],
        ...
        wp.on('aggregated', async ()=>{
            ...
            for (const fileName of sortedKnownFiles){
                ...
                if (middlewareFilePath) {
                    ...
                    _log.warnOnce(`The "${_constants1.MIDDLEWARE_FILENAME}" file convention is deprecated. Please use "${_constants1.PROXY_FILENAME}" instead.\n\n` + `  To migrate automatically, run:\n` + `  npx @next/codemod@canary middleware-to-proxy .\n\n` + `  Learn more: https://nextjs.org/docs/messages/middleware-to-proxy`);
                }
                ...
                if (isAppPath) {
                    ...
                    const normalizedPageName = (0, _normalizepathsep.normalizePathSep)(pageName);
                    ...
                    (0, _fileclassifier.addSlotIfNew)(slots, normalizedPageName);
                    // Handle layouts separately - they don't get added to appPaths
                    if (validFileMatcher.isAppLayoutPage(fileName)) {
                        const layoutRoute = (0, _ensureleadingslash.ensureLeadingSlash)((0, _apppaths.normalizeAppPath)(normalizedPageName).replace(/\/layout$/, ''));
                        layoutRoutes.push({
                            route: layoutRoute,
                            filePath: fileName
                        });
                    ...
                    const appRoute = (0, _normalizepathsep.normalizePathSep)(pageName);
                    ...
                    if (useFileSystemPublicRoutes) {
                        if (appDir && (0, _ismetadataroute.isStaticMetadataFile)(fileName.replace(appDir, ''))) {
                            ...
                            const normalizedPath = (0, _getmetadataroute.fillStaticMetadataSegment)(segment, lastSegment);
                    ...
                    const route = (0, _normalizepathsep.normalizePathSep)(pageName);
                    const routeEntry = {
                        route,
                        filePath: fileName
                    };
         ...
         */ if (/[\\\\/]_middleware$/.test(pageName)) {
            ...
            try {
                ...
                if (enabledTypeScript) {
                    ...
                    await (0, _rootparamstypeutils.writeRootParamsTypes)(routeTypesManifest, _path.default.join(distTypesDir, 'root-params.d.ts'), opts.nextConfig);
                }
                if (!resolved) {
                    resolve();
                    resolved = true;
...
async function setupDevBundler(opts) {
    ...
    const { installCodeFrameSupport } = require('../install-code-frame');
