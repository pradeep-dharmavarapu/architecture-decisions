...
async function collectPrefetchHints(fullPageDataBuffer, staleTime, clientModules, serverConsumerManifest, maxSize, maxBundleSize) {
    ...
    const [, headBuffer] = await renderSegmentPrefetch(buildId, staleTime, head, _segmentvalueencoding.HEAD_REQUEST_KEY, headVaryParams, clientModules, null);
    ...
    const subtreeHasRuntimePrefetch = (rootHints & _approutertypes.PrefetchHint.SubtreeHasRuntimePrefetch) !== 0;
    const headInlineState = {
        inlined: false
    };
    // Walk the tree with the parent-first, child-decides algorithm.
    const { node } = await collectPrefetchHintsImpl(flightRouterState, buildId, staleTime, seedData, clientModules, _segmentvalueencoding.ROOT_SEGMENT_REQUEST_KEY, null, maxSize, maxBundleSize, headGzipSize, headInlineState, subtreeHasRuntimePrefetch);
...
requestKey, parentGzipSize, maxSize, maxBundleSize, headGzipSize, headInlineState, routeHasRuntimePrefetch) {
    ...
    if (!isStaticPrefetchDisabled && seedData !== null) {
        ...
        const [, buffer] = await renderSegmentPrefetch(buildId, staleTime, seedData[0], requestKey, varyParams, clientModules, null);
    ...
    for(const parallelRouteKey in children){
        ...
        didInlineIntoChild ? null : sizeToOfferChild, maxSize, maxBundleSize, headGzipSize, headInlineState, routeHasRuntimePrefetch);
        if (slots === null) {
            slots = {};
        }
    ...
    const hasRuntimePrefetch = ((route[4] ?? 0) & _approutertypes.PrefetchHint.HasRuntimePrefetch) !== 0;
...
async function PrefetchTreeData({ isClientParamParsingEnabled, fullPageDataBuffer, serverConsumerManifest, clientModules, staleTime, segmentTasks, onCompletedProcessingRouteTree, prefetchInlining, hints }) {
    ...
    const tree = collectSegmentDataImpl(isClientParamParsingEnabled, flightRouterState, buildId, staleTime, seedData, clientModules, _segmentvalueencoding.ROOT_SEGMENT_REQUEST_KEY, segmentTasks, prefetchInlining, hints, null, headBundle);
    // Spawn a task to produce a prefetch response for the "head" segment,
    // unless it was inlined into a page's bundle.
    if (!headIsInlined) {
        segmentTasks.push((0, _scheduler.waitAtLeastOneReactRenderTask)().then(()=>renderSegmentPrefetch(buildId, staleTime, head, _segmentvalueencoding.HEAD_REQUEST_KEY, headVaryParams, clientModules, null)));
    }
...
function collectSegmentDataImpl(isClientParamParsingEnabled, route, buildId, staleTime, seedData, clientModules, requestKey, segmentTasks, prefetchInlining, hintTree, parentBundle, headBundle) {
    ...
    if (prefetchInlining && prefetchHints & _approutertypes.PrefetchHint.InlinedIntoChild) {
        ...
        if (seedData !== null && rsc !== null) {
            ...
            segmentTasks.push((0, _scheduler.waitAtLeastOneReactRenderTask)().then(()=>renderSegmentPrefetch(buildId, staleTime, rsc, requestKey, varyParams, clientModules, bundle)));
        }
    ...
    for(const parallelRouteKey in children){
        ...
        const childTree = collectSegmentDataImpl(isClientParamParsingEnabled, childRoute, buildId, staleTime, childSeedData, clientModules, childRequestKey, segmentTasks, prefetchInlining, childHintTree, childBundle, headBundle);
...
async function renderSegmentPrefetch(buildId, staleTime, rsc, requestKey, varyParams, clientModules, bundle) {
    ...
    const { prelude: segmentStream } = await (0, _static.prerender)(payload, clientModules, {
        filterStackFrame,
        signal: abortController.signal,
        onError: onSegmentPrerenderError
    });
