                    ...
                    case FetchStrategy.PPR:
                        {
                            ...
                            if (tree.prefetchHints & PrefetchHint.SubtreeHasRuntimePrefetch) {
                                ...
                                if (spawnedRuntimePrefetches !== null) {
                                    ...
                                    if (spawnedEntries.size > 0) {
                                        spawnPrefetchSubtask(fetchSegmentPrefetchesUsingDynamicRequest(task, route, FetchStrategy.PPRRuntime, MetadataOnlyRequestTree, spawnedEntries));
                                    }
                                }
                            }
                            return 2;
...
function pingStaticHead(now, task, route) {
    ...
    if (process.env.__NEXT_PREFETCH_INLINING && !(route.tree.prefetchHints & PrefetchHint.HeadOutlined)) {
        return;
    }
    const segments = {
        tree: route.metadata,
        entry: readOrCreateSegmentCacheEntry(now, FetchStrategy.PPR, route.metadata),
        parent: null
    };
...
function pingSharedPartOfCacheComponentsTree(now, task, route, oldTree, newTree, parentBundle) {
    ...
    if (newTreeChildren !== null) {
        for(const parallelRouteKey in newTreeChildren){
            ...
            const bundleForChild = process.env.__NEXT_PREFETCH_INLINING && bundleInProgress !== null && newTreeChild.prefetchHints & PrefetchHint.ParentInlinedIntoSelf ? bundleInProgress : null;
            let childExitStatus;
...
function pingNewPartOfCacheComponentsTree(now, task, route, tree, parentBundle) {
    ...
    if (tree.slots !== null) {
        ...
        for(const parallelRouteKey in tree.slots){
            ...
            // Only pass the bundle to the child that accepts it. A parent is
            // only ever bundled into one child.
            const bundleForChild = process.env.__NEXT_PREFETCH_INLINING && bundleInProgress !== null && childTree.prefetchHints & PrefetchHint.ParentInlinedIntoSelf ? bundleInProgress : null;
 ...
 */ function pingSegmentBundle(now, task, route, routeKey, tree, segments) {
        ...
        switch(nodeEntry.status){
            ...
            case EntryStatus.Pending:
                switch(nodeEntry.fetchStrategy){
                    ...
                    case FetchStrategy.LoadingBoundary:
                        if (background(task)) {
                            const revalidatingEntry = readOrCreateRevalidatingSegmentEntry(now, FetchStrategy.PPR, nodeTree);
    ...
    spawnPrefetchSubtask(fetchSegmentsOnCacheMiss(route, routeKey, tree, segments, segmentCount));
}
/**
 ...
 */ function accumulateSegmentBundle(now, task, route, tree, parentBundle) {
    ...
    const segment = readOrCreateSegmentCacheEntry(now, task.fetchStrategy, tree);
    if (process.env.__NEXT_PREFETCH_INLINING && tree.prefetchHints & PrefetchHint.InlinedIntoChild) {
        return {
            tree,
            entry: segment,
            parent: parentBundle
    ...
    if (process.env.__NEXT_PREFETCH_INLINING && tree.prefetchHints & PrefetchHint.HeadInlinedIntoSelf) {
        effectiveParent = {
            tree: route.metadata,
            entry: readOrCreateSegmentCacheEntry(now, FetchStrategy.PPR, route.metadata),
            parent: parentBundle
