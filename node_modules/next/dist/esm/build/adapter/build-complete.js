    ...
    if (typeof adapterMod.onBuildComplete === 'function') {
        ...
        if (config.output === 'export') {
            ...
            if (appPageKeys) {
                for (const page of appPageKeys){
                    ...
                    if (existingOutput) {
                        ...
                        await pushAsset(existingOutput.assets, existingOutput.assetsHashes, path.relative(tracingRoot, pageFile), pageFile, bundler);
                        continue;
                    }
            ...
            for(const dynamicRoute in prerenderManifest.dynamicRoutes){
                ...
                const partialFallback = isAppPage && remainingPrerenderableParams !== undefined && remainingPrerenderableParams.length > 0 && renderingMode === RenderingMode.PARTIALLY_STATIC && typeof fallback === 'string' && Boolean(meta.postponed);
...
async function getSharedNodeAssets({ dir, bundler, distDir, tracingRoot, requiredServerFiles, hasInstrumentationHook }) {
    ...
    for (const type of moduleTypes){
        ...
        for (const dependencyPath of currentDependencies){
            ...
            if (type === 'pages') {
                await pushAsset(pagesSharedNodeAssets, pagesSharedNodeAssetsHashes, rootRelativeFilePath, path.join(tracingRoot, rootRelativeFilePath), bundler);
            } else {
                await pushAsset(appPagesSharedNodeAssets, appPagesSharedNodeAssetsHashes, rootRelativeFilePath, path.join(tracingRoot, rootRelativeFilePath), bundler);
            }
        }
    }
    ...
    await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, path.relative(tracingRoot, setupNodeStubPath), require.resolve('next/dist/build/adapter/setup-node-env.external'), bundler);
    if (bundler !== Bundler.Turbopack) {
        ...
        for (const rootRelativeFilePath of fileList){
            await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, rootRelativeFilePath, path.join(tracingRoot, rootRelativeFilePath), bundler);
        }
    }
    if (hasInstrumentationHook) {
        const { entryHash: instrumentationEntryHash } = await loadNFT(sharedNodeAssets, sharedNodeAssetsHashes, tracingRoot, path.join(distDir, 'server', 'instrumentation.js.nft.json'));
        const fileOutputPath = path.relative(tracingRoot, path.join(distDir, 'server', 'instrumentation.js'));
        await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, fileOutputPath, path.join(distDir, 'server', 'instrumentation.js'), bundler, instrumentationEntryHash);
    }
