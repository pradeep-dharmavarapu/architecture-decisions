...
function toWebReadableStream(stream) {
    ...
    const { Readable: ReadableClass } = require('node:stream');
    return ReadableClass.toWeb(stream);
}
...
export function connectReactDebugChannel(requestId, debugChannel, sendToClient) {
    const reader = toWebReadableStream(debugChannel.readable).pipeThrough(// We're sending the chunks in batches to reduce overhead in the browser.
