 ...
 */ async function generateDynamicFlightRenderResultWithStagesInDev(req, ctx, initialRequestStore, createRequestStore, fallbackParams) {
    ...
    const shouldValidate = !isBypassingCachesInDev(initialRequestStore, workStore) && (initialRequestStore.isHmrRefresh === true || await anySegmentNeedsInstantValidationInDev(loaderTree));
    ...
    !isBypassingCachesInDev(initialRequestStore, workStore)) {
        ...
        if (shouldValidate) {
            ...
            consoleAsyncStorage.run({
                dim: true
            }, spawnStaticShellValidationInDev, accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, ctx, finalRequestStore, fallbackParams, validationDebugChannelClient, didErrorObservably);
        } else {
            ...
            void accumulatedChunksPromise.then(()=>{
                ...
                if (invalidDynamicUsageError && !invalidDynamicUsageError.digest) {
 ...
 */ async function spawnStaticShellValidationInDevImpl(accumulatedChunksPromise, syncInterruptReason, startTime, staticStageEndTime, runtimeStageEndTime, ctx, requestStore, fallbackRouteParams, debugChannelClient, devRenderDidError) {
    ...
    if (invalidDynamicUsageError) {
        ...
        // emitted it as a Flight error chunk — surfacing it again here would
        // duplicate the entry in the dev overlay.
        if (!invalidDynamicUsageError.digest) {
    ...
    if (needsInstantValidation) {
        const instantConfigsResult = await validateInstantConfigs(accumulatedChunks, debugChunks, startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples, devRenderDidError);
...
async function validateStagedShell(stageChunks, allServerChunks, debugChunks, debugEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, trackDynamicHole) {
    ...
    try {
        ...
        return getStaticShellDisallowedDynamicReasons(workStore, preludeIsEmpty ? PreludeState.Empty : PreludeState.Full, dynamicValidation, allowEmptyStaticShell);
    } catch (thrownValue) {
        // Even if the root errors we still want to report any cache components errors
        // that were discovered before the root errored.
        let errors = getStaticShellDisallowedDynamicReasons(workStore, PreludeState.Errored, dynamicValidation, allowEmptyStaticShell);
 ...
 */ async function validateInstantConfigs(accumulatedChunks, debugChunks, startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples, devRenderDidError) {
    const debug = process.env.NEXT_PRIVATE_DEBUG_VALIDATION === '1' ? console.log : undefined;
    ...
    async function validateAtDepthImpl(depth, groupDepthForValidation, previousBoundaryState) {
        ...
        try {
            ...
            const { preludeIsEmpty } = await processPreludeOp(unprocessedPrelude);
            result = getNavigationDisallowedDynamicReasons(workStore, preludeIsEmpty ? PreludeState.Empty : PreludeState.Full, instantValidationState, validationSampleTracking, boundaryState, devRenderDidError);
        } catch (thrownValue) {
            result = getNavigationDisallowedDynamicReasons(workStore, PreludeState.Errored, instantValidationState, validationSampleTracking, boundaryState, devRenderDidError);
        }
...
async function validateInstantConfigInBuildWithSample(outerCtx, sample, allPossibleFallbackRouteParams, prefilledDataCache) {
    ...
    return workAsyncStorage.run(workStore, async ()=>{
        ...
        return await validateInstantConfigs(accumulatedChunks, debugChunks, startTime, sampleRootParams, fallbackRouteParams, validationCtx, undefined, validationSamples, false // build has no shared dev render that would surface errors
        );
    });
}
/**
...
async function prerenderToStream(req, res, ctx, metadata, tree, fallbackRouteParams) {
    ...
    const { cachedNavigations } = renderOpts.experimental;
    const renderFlightStream = process.env.__NEXT_USE_NODE_STREAMS ? renderToNodeFlightStream : renderToWebFlightStream;
    const renderFizzStream = process.env.__NEXT_USE_NODE_STREAMS ? renderToNodeFizzStream : renderToWebFizzStream;
    const createInlinedDataStream = process.env.__NEXT_USE_NODE_STREAMS ? createNodeInlinedDataStream : createWebInlinedDataStream;
    ...
    try {
       ...
       */ // The prerender controller represents the lifetime of the prerender. It
            ...
            throwIfDisallowedDynamic(workStore, preludeIsEmpty ? PreludeState.Empty : PreludeState.Full, dynamicValidation, serverDynamicTracking, allowEmptyStaticShell);
            const getServerInsertedHTML = makeGetServerInsertedHTML({
                polyfills,
                renderServerInsertedHTML,
                serverCapturedErrors: allCapturedErrors,
            ...
            if (serverIsDynamic) {
                ...
                if (hasFallbackRouteParams) {
                    ...
                    const emptyReactServerResult = await createReactServerPrerenderResultFromRender(renderFlightStream(ComponentMod, [], clientModules, {
                        filterStackFrame,
                        onError: serverComponentsErrorHandler
                    }));
            ...
            reactServerResult = reactServerPrerenderResult = await createReactServerPrerenderResultFromRender(workUnitAsyncStorage.run(reactServerPrerenderStore, renderFlightStream, ComponentMod, RSCPayload, clientModules, {
                filterStackFrame,
                onError: serverComponentsErrorHandler
            }));
       ...
       */ // First we check if we have any dynamic holes in our HTML prerender
            ...
            let reactServerResult;
            reactServerResult = reactServerPrerenderResult = await createReactServerPrerenderResultFromRender(workUnitAsyncStorage.run(prerenderLegacyStore, renderFlightStream, ComponentMod, RSCPayload, clientModules, {
                filterStackFrame,
                onError: serverComponentsErrorHandler
            }));
            const { stream: htmlStream } = await workUnitAsyncStorage.run(prerenderLegacyStore, renderFizzStream, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
            /*#__PURE__*/ _jsx(App, {
                reactServerStream: reactServerResult.asUnclosingStream(),
                reactDebugStream: undefined,
                debugEndTime: undefined,
        ...
        const errorServerStreamRaw = workUnitAsyncStorage.run(prerenderLegacyStore, renderFlightStream, ComponentMod, errorRSCPayload, clientModules, {
            filterStackFrame,
            onError: serverComponentsErrorHandler
        });
        ...
        try {
            const { stream: errorHtmlStream } = await workUnitAsyncStorage.run(prerenderLegacyStore, renderFizzStream, // eslint-disable-next-line @next/internal/no-ambiguous-jsx
