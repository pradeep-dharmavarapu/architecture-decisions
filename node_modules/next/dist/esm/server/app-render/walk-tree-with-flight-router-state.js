 ...
 */ export async function walkTreeWithFlightRouterState({ loaderTreeToFilter, parentParams, flightRouterState, parentIsInsideSharedLayout, rscHead, injectedCSS, injectedJS, injectedFontPreloadTags, rootLayoutIncluded, ctx, preloadCallbacks, MetadataOutlet, hintTree }) {
    const { renderOpts: { nextFontManifest, experimental }, query, isPrefetch, getDynamicParamFromSegment, parsedRequestHeaders, workStore } = ctx;
    ...
    const isBuildTimePrerendering = ctx.renderOpts.isBuildTimePrerendering ?? false;
   ...
   */ const renderComponentsOnThisLevel = // No further router state available
    ...
    !matchSegment(actualSegment, flightRouterState[0]) || // Explicit refresh
    ...
    isPrefetch && !Boolean(modules.loading) && !hasLoadingComponentInTree(loaderTreeToFilter))) {
        ...
        const routerState = parsedRequestHeaders.isRouteTreePrefetchRequest ? await createRouteTreePrefetch(loaderTreeToFilter, hintTree, prefetchInliningEnabled, cacheComponents, isStaticGeneration, isBuildTimePrerendering, getDynamicParamFromSegment) : await createFlightRouterStateFromLoaderTree(loaderTreeToFilter, hintTree, prefetchInliningEnabled, cacheComponents, isStaticGeneration, isBuildTimePrerendering, getDynamicParamFromSegment, query);
        return [
            [
                overriddenSegment,
                routerState,
    ...
    if (renderComponentsOnThisLevel) {
        ...
        // tracked by the client? Dig deeper to see if we can simplify this.
        canSegmentBeOverridden(actualSegment, flightRouterState[0]) ? flightRouterState[0] : actualSegment;
        const routerState = await createFlightRouterStateFromLoaderTree(// Create router state using the slice of the loaderTree
        loaderTreeToFilter, hintTree, prefetchInliningEnabled, cacheComponents, isStaticGeneration, isBuildTimePrerendering, getDynamicParamFromSegment, query);
    ...
    if (layoutPath) {
        getLinkAndScriptTags(layoutPath, injectedCSSWithCurrentLayout, injectedJSWithCurrentLayout, true);
 ...
 */ export async function createFullTreeFlightDataForNavigation({ loaderTree, rscHead, injectedCSS, injectedJS, injectedFontPreloadTags, ctx, preloadCallbacks, MetadataOutlet }) {
    var _ctx_renderOpts_prefetchHints;
    const { renderOpts: { experimental }, query, getDynamicParamFromSegment, pagePath, workStore: workStoreForInitialRender } = ctx;
    ...
    const routerState = await createFlightRouterStateFromLoaderTree(loaderTree, hintTreeForInitialRender, Boolean(experimental.prefetchInlining), ctx.renderOpts.cacheComponents, workStoreForInitialRender.isStaticGeneration, ctx.renderOpts.isBuildTimePrerendering ?? false, getDynamicParamFromSegment, query);
 ...
 */ const canSegmentBeOverridden = (existingSegment, segment)=>{
    ...
    return ((_getSegmentParam = getSegmentParam(existingSegment)) == null ? void 0 : _getSegmentParam.paramName) === segment[0];
};
