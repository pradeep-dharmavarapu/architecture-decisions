...
import { fillStaticMetadataSegment, normalizeMetadataPageToRoute } from '../../../lib/metadata/get-metadata-route';
...
import { addSlotIfNew } from '../../../build/file-classifier';
...
async function startWatcher(opts) {
    ...
    if (opts.nextConfig.experimental.lockDistDir) {
        ...
        lockfile = await Lockfile.acquireWithRetriesOrExit(path.join(distDir, 'lock'), 'next dev', true, JSON.stringify(serverInfo), opts.dir, opts.nextConfig.distDir);
    }
    ...
    await fs.promises.writeFile(prerenderManifestPath, JSON.stringify({
        version: 4,
        routes: {},
        dynamicRoutes: {},
        notFoundRoutes: [],
            ...
            for (const fileName of sortedKnownFiles){
                ...
                if (isAppPath) {
                    ...
                    if (validFileMatcher.isAppLayoutPage(fileName)) {
                        const layoutRoute = ensureLeadingSlash(normalizeAppPath(normalizedPageName).replace(/\/layout$/, ''));
                        layoutRoutes.push({
                            route: layoutRoute,
                            filePath: fileName
                        });
                    ...
                    if (useFileSystemPublicRoutes) {
                        if (appDir && isStaticMetadataFile(fileName.replace(appDir, ''))) {
                            ...
                            const normalizedPath = fillStaticMetadataSegment(segment, lastSegment);
         ...
         */ if (/[\\\\/]_middleware$/.test(pageName)) {
            ...
            try {
                ...
                if (enabledTypeScript) {
                    ...
                    await writeRootParamsTypes(routeTypesManifest, path.join(distTypesDir, 'root-params.d.ts'), opts.nextConfig);
                }
                if (!resolved) {
                    resolve();
                    resolved = true;
