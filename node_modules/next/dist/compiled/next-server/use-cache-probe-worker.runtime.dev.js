...
`+("time"===type?"  - Measure elapsed time with `performance.now()` instead of `Date.now()`\n":"")+`
`+`Learn more: ${docsUrl}`),"__NEXT_ERROR_CODE",{value:"E1242",enumerable:!1,configurable:!0})}function io(expression,type){var route;let workUnitStore=work_unit_async_storage_external_js_namespaceObject.workUnitAsyncStorage.getStore(),workStore=work_async_storage_external_js_namespaceObject.workAsyncStorage.getStore();if(workUnitStore&&workStore)switch(workUnitStore.type){case"prerender":case"prerender-runtime":!1===workUnitStore.controller.signal.aborted&&abortOnSynchronousPlatformIOAccess(workStore.route,expression,applyOwnerStack(createSyncIOErrorImpl(workStore.route,expression,type,SYNC_IO_DOCS[type])),workUnitStore);break;case"prerender-client":!1===workUnitStore.controller.signal.aborted&&abortOnSynchronousPlatformIOAccess(workStore.route,expression,applyOwnerStack((route=workStore.route,Object.defineProperty(Error(`Route "${route}": Next.js encountered ${expression} in a Client Component.
