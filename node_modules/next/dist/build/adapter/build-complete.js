    ...
    if (typeof adapterMod.onBuildComplete === 'function') {
        ...
        if (config.output === 'export') {
            ...
            async function handleTraceFiles(entryFilePath, type) {
                const assets = {};
                const assetsHashes = {};
                const { entryHash } = await loadNFT(assets, assetsHashes, tracingRoot, `${entryFilePath}.nft.json`);
                ...
                if (entryHash) {
                    assetsHashes[_path.default.relative(tracingRoot, entryFilePath)] = entryHash;
                }
                return {
                    assets,
                    assetsHashes,
            ...
            if (appPageKeys) {
                for (const page of appPageKeys){
                    ...
                    if (existingOutput) {
                        ...
                        await pushAsset(existingOutput.assets, existingOutput.assetsHashes, _path.default.relative(tracingRoot, pageFile), pageFile, bundler);
                        continue;
                    }
            ...
            for(const dynamicRoute in prerenderManifest.dynamicRoutes){
                ...
                const partialFallback = isAppPage && remainingPrerenderableParams !== undefined && remainingPrerenderableParams.length > 0 && renderingMode === _renderingmode.RenderingMode.PARTIALLY_STATIC && typeof fallback === 'string' && Boolean(meta.postponed);
...
async function getSharedNodeAssets({ dir, bundler, distDir, tracingRoot, requiredServerFiles, hasInstrumentationHook }) {
    ...
    for (const type of moduleTypes){
        ...
        for (const dependencyPath of currentDependencies){
            ...
            if (type === 'pages') {
                await pushAsset(pagesSharedNodeAssets, pagesSharedNodeAssetsHashes, rootRelativeFilePath, _path.default.join(tracingRoot, rootRelativeFilePath), bundler);
            } else {
                await pushAsset(appPagesSharedNodeAssets, appPagesSharedNodeAssetsHashes, rootRelativeFilePath, _path.default.join(tracingRoot, rootRelativeFilePath), bundler);
            }
        }
    }
    ...
    await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, _path.default.relative(tracingRoot, setupNodeStubPath), require.resolve('next/dist/build/adapter/setup-node-env.external'), bundler);
    if (bundler !== _bundler.Bundler.Turbopack) {
        ...
        for (const rootRelativeFilePath of fileList){
            await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, rootRelativeFilePath, _path.default.join(tracingRoot, rootRelativeFilePath), bundler);
        }
    }
    if (hasInstrumentationHook) {
        const { entryHash: instrumentationEntryHash } = await loadNFT(sharedNodeAssets, sharedNodeAssetsHashes, tracingRoot, _path.default.join(distDir, 'server', 'instrumentation.js.nft.json'));
        const fileOutputPath = _path.default.relative(tracingRoot, _path.default.join(distDir, 'server', 'instrumentation.js'));
        await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, fileOutputPath, _path.default.join(distDir, 'server', 'instrumentation.js'), bundler, instrumentationEntryHash);
    }
    ...
    for (const file of requiredServerFiles){
        ...
        await pushAsset(sharedNodeAssets, sharedNodeAssetsHashes, fileOutputPath, filePath, bundler);
    }
    return {
        sharedNodeAssets,
...
async function pushAsset(assets, assetsHashes, targetFilePath, sourceFilePath, bundler, hashOverride) {
    if (!(targetFilePath in assets)) {
        assets[targetFilePath] = sourceFilePath;
        if (bundler === _bundler.Bundler.Turbopack) {
            assetsHashes[targetFilePath] = hashOverride ?? await hashFile(sourceFilePath);
        }
    }
}
async function loadNFT(assets, assetsHashes, tracingRoot, traceFilePath) {
    const { files, fileHashes, entryHash } = await JSON.parse(await _promises.default.readFile(traceFilePath, 'utf8'));
