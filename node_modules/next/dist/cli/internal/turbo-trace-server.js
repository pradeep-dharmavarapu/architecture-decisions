...
const { McpServer } = require('next/dist/compiled/@modelcontextprotocol/sdk/server/mcp');
const { StreamableHTTPServerTransport } = require('next/dist/compiled/@modelcontextprotocol/sdk/server/streamableHttp');
const DEFAULT_WS_PORT = 5747;
/** 100 internal ticks = 1 µs */ const TICKS_PER_US = 100;
...
async function startTurboTraceServerCli(file, port, mcpPort) {
    const wsPort = port ?? DEFAULT_WS_PORT;
    const httpPort = mcpPort ?? wsPort + 1;
    let bindings;
    try {
        bindings = await (0, _swc.loadBindings)();
    } catch  {
        console.error('Error: Could not load native bindings. The trace server requires native (non-WASM) bindings.');
        process.exit(1);
    }
    let handle;
    try {
        ...
        console.error(`Error: Could not start trace server for "${file}": ${err instanceof Error ? err.message : err}`);
        process.exit(1);
    }
    console.log(`Turbopack trace server started. View trace at https://trace.nextjs.org?port=${wsPort}`);
    ...
    mcpServer.registerTool('query_spans', {
        description: 'Query spans from a turbopack trace file. Returns spans with timing, CPU usage, and attribute details. Set `outputType` to "json" for machine-readable output or "markdown" (default) for human-readable output. Use the `parent` parameter (with an ID from a previous result) to drill into children. Results are paginated to 20 spans per page.',
        inputSchema: {
            parent: _zod.default.string().optional().describe('Span ID to enumerate children of. Omit for root-level spans. Use the `id` field from a previous result.'),
            aggregated: _zod.default.boolean().optional().describe('When true (default), aggregate spans with the same name into a single entry. Set to false to see individual raw spans.'),
            sort: _zod.default.enum([
                'value',
                'name'
            ]).optional().describe('Sort mode: "value" for corrected duration descending, "name" for alphabetical. Omit for execution order.'),
            search: _zod.default.string().optional().describe('Substring search query applied to span name and category.'),
            page: _zod.default.number().optional().describe('1-based page number. Default 1.'),
            outputType: _zod.default.enum([
                'markdown',
                'json'
            ]).optional().describe('Output format. "markdown" (default) returns human-readable markdown. "json" returns structured JSON with all span fields.')
        }
    }, (args)=>{
        ...
        const parentLabel = args.parent ? `children of ID \`${args.parent}\`` : 'root level';
    ...
    server.on('error', (err)=>{
        if (err.code === 'EADDRINUSE') {
            console.error(`Error: MCP port ${httpPort} is already in use. Use --mcp-port to specify a different port.`);
        } else {
    ...
    server.listen(httpPort, '127.0.0.1', ()=>{
        console.log(`Query this trace from the command line: next internal query-trace --help`);
        console.log(`Alternatively, connect an MCP client to http://127.0.0.1:${httpPort}/mcp`);
    });
}
