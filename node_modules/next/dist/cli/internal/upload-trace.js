...
Object.defineProperty(exports, "uploadTraceToBlob", {
    enumerable: true,
    get: function() {
        return uploadTraceToBlob;
    }
});
...
function createProgressStream(content, onProgress) {
    let offset = 0;
...
function validateCpuProfile(header, file) {
    if (header.length < CPUPROFILE_HEADER.length || !header.subarray(0, CPUPROFILE_HEADER.length).equals(CPUPROFILE_HEADER)) {
        console.error(`Error: ${file} does not appear to be a valid V8 CPU profile.`);
        process.exit(1);
    }
}
function validateTurbopackTrace(header, file) {
    if (header.length < TURBOPACK_TRACE_HEADER.length || !header.subarray(0, TURBOPACK_TRACE_HEADER.length).equals(TURBOPACK_TRACE_HEADER)) {
        console.error(`Error: ${file} does not appear to be a valid Turbopack trace (missing TRACEv0 header).`);
        process.exit(1);
    }
}
async function uploadTraceToBlob(options) {
    const dir = options.directory || process.cwd();
    const profilesDir = _path.default.join(dir, '.next-profiles');
    let entries;
    try {
        entries = await _promises.default.readdir(profilesDir);
    } catch (err) {
        if (err.code === 'ENOENT') {
            console.error(`Error: Profiles directory not found at ${profilesDir}. Run "next build --experimental-cpu-prof" or "next build --internal-trace" first.`);
            process.exit(1);
        }
        throw err;
    }
    const uploadableFiles = entries.filter((f)=>f.endsWith('.cpuprofile') || f.endsWith('trace-turbopack'));
    ...
    const { put } = require('next/dist/compiled/@vercel/blob');
    let sessionId;
    let sessionToken;
    let uploadedCount = 0;
    for (const file of uploadableFiles){
        const filePath = _path.default.join(profilesDir, file);
        const stat = await _promises.default.stat(filePath);
        if (stat.size === 0) {
            console.warn(`Skipping ${file}: file is empty.`);
            continue;
        }
        const fd = await _promises.default.open(filePath, 'r');
        ...
        if (!tokenRes.ok) {
            console.error(`Error: Failed to get upload token for ${file} (${tokenRes.status} ${tokenRes.statusText})`);
            process.exit(1);
        }
